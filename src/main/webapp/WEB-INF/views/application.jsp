   ...
   <main class="main-container">
      ...
      <form action="#" method="post" onsubmit="return false;">
		...
		<div class="form-section">
			...
			<!-- 출생 유형 선택 (기본 선택 없음) -->
			<div class="input-field radio-group">
				<input type="radio" name="birthType" value="born"><label>출생일</label>
				<input type="radio" name="birthType" value="expected"><label>출산예정일</label>
			</div>
			...
			<div id="born-fields" style="display: none;">
				...
				<div class="form-group">
					...
					<div class="input-field">
						<input type="text" id="child-rrn" name="child_resi_regi_number"
				...
				<div class="form-group">
					...
					<div class="input-field">
						<input type="date" id="birth-date" name="child_birth_date">
					</div>
				</div>
			</div>
			...
			<div id="expected-fields" style="display: none;">
				<div class="form-group">
					...
					<div class="input-field">
						<input type="date" id="expected-date" name="expected_birth_date">
					</div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // 시작일 선택 → 종료일 입력 영역 표시 + 종료일 min 세팅
  ...
  generateBtn.addEventListener('click', function() {
    ...
    while (currentStartDate <= endDate) {
      // currentStartDate 기준 한 달 구간 끝 = (다음달 같은 일 - 1일)
      var periodEndDate = new Date(currentStartDate.getFullYear(), currentStartDate.getMonth() + 1, 0);

      // 시작일이 해당 월 중간이면 그 날부터
      var displayStart = new Date(currentStartDate.getTime());
      // 종료일이 해당 월 말보다 작으면 종료일로
      var displayEnd = periodEndDate > endDate ? new Date(endDate.getTime()) : periodEndDate;

      var dateRangeText = formatDate(displayStart) + ' ~ ' + formatDate(displayEnd);
  ...
  // ====== 자녀정보 라디오/예정일 min 설정 ======
  (function () {
    var radios = document.querySelectorAll('input[name="birthType"]');
    var bornWrap = document.getElementById('born-fields');
    var expectedWrap = document.getElementById('expected-fields');

    var nameInput  = document.getElementById('child-name');
    var rrnInput   = document.getElementById('child-rrn');
    var birthInput = document.getElementById('birth-date');
    var expInput   = document.getElementById('expected-date');
    ...
    if (expInput) {
      ...
      expInput.setAttribute('min', minStr);
    }

    function setBornRequired(on){ if(nameInput) nameInput.required=on; if(rrnInput) rrnInput.required=on; if(birthInput) birthInput.required=on; }
    function setExpectedRequired(on){ if(expInput) expInput.required=on; }
    function clearBorn(){ if(nameInput) nameInput.value=''; if(rrnInput) rrnInput.value=''; if(birthInput) birthInput.value=''; }
    function clearExpected(){ if(expInput) expInput.value=''; }

    function updateView(){
      var checked = document.querySelector('input[name="birthType"]:checked');
      if(!checked){ bornWrap.style.display='none'; expectedWrap.style.display='none'; setBornRequired(false); setExpectedRequired(false); return; }
      if(checked.value==='born'){ bornWrap.style.display=''; expectedWrap.style.display='none'; setBornRequired(true); setExpectedRequired(false); clearExpected(); }
      else { bornWrap.style.display='none'; expectedWrap.style.display=''; setBornRequired(false); setExpectedRequired(true); clearBorn(); }
    }
