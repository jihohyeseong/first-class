...
:root{
	...
	--danger-color: #dc3545; /* [추가] 위험/경고 색상 */
...
<main class="main-container">
    ...
    <div class="info-table-container">
		...
		<table class="info-table table-4col">
			<tbody>
				<tr>
					<th>접수번호</th><td><c:out value="${app.applicationNumber}" /></td>
					<th>민원내용</th><td>육아휴직 급여 신청</td>
				</tr>
				<tr>
					<th>신청일</th>
					<td><fmt:formatDate value="${app.submittedDt}" pattern="yyyy-MM-dd" /></td>
					<th>신청인</th><td><c:out value="${userDTO.username}"/></td>
				</tr>
			</tbody>
		</table>
	</div>
	...
	<div class="info-table-container">
		...
		<table class="info-table table-4col">
			<tbody>
				<tr>
					<th>이름</th><td colspan="3"><c:out value="${userDTO.name}" /></td>
				</tr>
				<tr>
					<th>주민등록번호</th>
					<td colspan="3">
				...
				<tr>
					<th>휴대전화번호</th><td colspan="3"><c:out value="${userDTO.phoneNumber}" /></td>
				</tr>
				<tr>
					<th>주소</th>
	...
	<div class="info-table-container">
		...
		<table class="info-table table-4col">
			<tbody>
				<tr>
					<th>사업장 <br>동의여부</th>
					<%-- [수정] 사업장 동의여부 '아니요'일 때 하이라이팅 --%>
					<td>
						<c:choose>
							<c:when test="${app.businessAgree == 'Y'}">예</c:when>
							<c:when test="${app.businessAgree == 'N'}">
								<c:if test="${isAdmin}"><span class="highlight-warning">아니요</span></c:if>
					...
					<th>사업장 이름</th>
					<td>
						<%-- [수정] 사업장 이름 비어있을 때 하이라이팅 --%>
						<c:if test="${isAdmin and empty app.businessName}"><span class="highlight-warning">미입력</span></c:if>
						<c:if test="${!isAdmin or not empty app.businessName}">${app.businessName}</c:if>
					</td>
				</tr>
				<tr>
					<th>사업장 <br>등록번호</th>
					<td>
						<%-- [수정] 사업자번호 비어있을 때 하이라이팅 --%>
						<c:if test="${isAdmin and empty app.businessRegiNumber}"><span class="highlight-warning">미입력</span></c:if>
						<c:if test="${!isAdmin or not empty app.businessRegiNumber}">${app.businessRegiNumber}</c:if>
					</td>
					<th>인사담당자 <br>연락처</th><td>02-9876-5432</td>
				</tr>
				<tr>
					<th>사업장 주소</th>
					<td colspan="3">
						<%-- [수정] 사업장 주소 비어있을 때 하이라이팅 --%>
						<c:choose>
							<c:when test="${empty app.businessZipNumber and empty app.businessAddrBase}">
							...
							<c:otherwise>
								...
								<c:if test="${not empty app.businessAddrDetail}">&nbsp;<c:out value="${app.businessAddrDetail}"/></c:if>
							</c:otherwise>
						</c:choose>
					</td>
				</tr>
	...
	<div class="info-table-container">
		...
		<table class="info-table table-4col">
			<tbody>
				<tr>
					...
					<td>
						<%-- [수정] 시작일 비어있을 때 하이라이팅 --%>
						<c:if test="${isAdmin and empty app.startDate}"><span class="highlight-warning">미입력</span></c:if>
						<c:if test="${!isAdmin or not empty app.startDate}">${app.startDate}</c:if>
					</td>
					<th>총 휴직 기간</th>
					<%-- [수정] JavaScript로 하이라이팅하기 위해 id 추가 --%>
					<td id="total-leave-period">
						(${empty app.startDate ? '미입력' : app.startDate} ~ ${empty app.endDate ? '미입력' : app.endDate})
					</td>
				</tr>
				<tr>
					<th>월별 분할 <br>신청 여부</th><td colspan="3">아니오 (일괄 신청)</td>
				</tr>
			</tbody>
		</table>
		...
		<table class="info-table table-4col">
			<thead><tr><th>회차</th><th>기간</th><th>사업장 지급액</th><th>정부 지급액</th><th>지급예정일</th></tr></thead>
			<tbody>
				...
				<c:if test="${empty terms}">
					<tr><td colspan="5" style="text-align: center; color: #888;">단위기간 내역이 없습니다.</td></tr>
				</c:if>
			</tbody>
		</table>
	</div>
				...
				<c:choose>
						...
						<tr>
							...
							<td colspan="3">
								<c:choose>
									...
									<c:otherwise><fmt:formatDate value="${app.childBirthDate}" pattern="yyyy.MM.dd"/></c:otherwise>
								</c:choose>
							</td>
						</tr>
					</c:when>
					<c:otherwise>
						<tr>
							<th>자녀 이름</th>
							<td>
								<c:if test="${isAdmin and empty app.childName}"><span class="highlight-warning">미입력</span></c:if>
								<c:if test="${!isAdmin or not empty app.childName}"><c:out value="${app.childName}" /></c:if>
							</td>
							<th>출산일</th>
							<td>
								<c:if test="${isAdmin and empty app.childBirthDate}"><span class="highlight-warning">미입력</span></c:if>
								<c:if test="${!isAdmin or not empty app.childBirthDate}"><fmt:formatDate value="${app.childBirthDate}" pattern="yyyy.MM.dd"/></c:if>
							</td>
						</tr>
						<tr>
							<th>주민등록번호</th>
							<td colspan="3">
								<c:if test="${isAdmin and empty app.childResiRegiNumber}"><span class="highlight-warning">미입력</span></c:if>
								<c:if test="${!isAdmin or not empty app.childResiRegiNumber}"><c:out value="${fn:substring(app.childResiRegiNumber, 0, 7)}" />******</c:if>
							</td>
						</tr>
					</c:otherwise>
				</c:choose>
		...
		<table class="info-table table-4col">
			<tbody>
				<tr>
					<th>은행</th>
					<td>
						<c:if test="${isAdmin and empty app.bankName}"><span class="highlight-warning">미입력</span></c:if>
						<c:if test="${!isAdmin or not empty app.bankName}"><c:out value="${app.bankName}"/></c:if>
					</td>
					<th>계좌번호</th>
					<td>
				...
				<tr>
					<th>예금주 이름</th><td colspan="3"><c:out value="${userDTO.name}"/></td>
				</tr>
			</tbody>
		</table>
	</div>
		...
		<table class="info-table table-4col">
			<tbody>
				<tr>
					...
					<th>대표전화</th><td>02-2004-7301</td>
				</tr>
				<tr>
					<th>주소</th><td colspan="3">서울 중구 삼일대로363 1층 (장교동)</td>
				</tr>
			</tbody>
		</table>
	</div>
	...
	<div class="info-table-container">
		...
		<table class="info-table table-4col">
			<tbody>
				<tr>
					<th>부정수급 <br>안내 확인</th>
					<td colspan="3"><span class="success-text">확인 및 동의 완료</span> (2025.10.02)</td>
				</tr>
			</tbody>
		</table>
	</div>
	...
	<div class="button-container">
		...
		<c:choose>
			...
			<c:otherwise>
				<a href="${pageContext.request.contextPath}/main" class="btn bottom-btn btn-secondary">목록으로 돌아가기</a>
				<c:choose>
					<c:when test="${isSubmitted}">
						<button class="btn bottom-btn btn-secondary" style="margin-left: 15px;" disabled>제출 완료</button>
					</c:when>
					<c:otherwise>
						<a href="${pageContext.request.contextPath}/apply/edit?appNo=${app.applicationNumber}" class="btn bottom-btn btn-primary" style="margin-left: 15px;">신청 내용 수정</a>
						...
						<form action="${pageContext.request.contextPath}/apply/delete" method="post" style="display: inline;">
							<sec:csrfInput />
							<input type="hidden" name="appNo" value="${app.applicationNumber}" />
							<button type="submit" class="btn bottom-btn btn-secondary" style="margin-left: 15px;" onclick="return confirm('정말 삭제하시겠습니까?');">삭제</button>
						</form>
					</c:otherwise>
				</c:choose>
			</c:otherwise>
...
<%-- [수정] 관리자일 때만 실행되는 하이라이팅 스크립트 --%>
<c:if test="${isAdmin}">
	<script>
		document.addEventListener('DOMContentLoaded', function() {
			// 4. 휴직 기간이 1년을 초과하는지 검사
			const startDateStr = '${app.startDate}';
			const endDateStr = '${app.endDate}';
			...
			if (startDateStr && endDateStr) {
				...
				// 밀리초를 일(day)로 변환 (시작일 포함을 위해 +1)
				const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)) + 1;

				if (diffDays > 365) {
					const periodCell = document.getElementById('total-leave-period');
					if (periodCell) {
						periodCell.classList.add('highlight-warning');
					}
				}
			}
		});
...
<script>
	$(function() {
	    ...
	    $('#reject-btn').on('click', function(event) {
	        ...
	        $.ajax({
	            ...
	            success: function(data) {
	                ...
	                rejectCodeSelect.html('<option value="">사유를 선택하세요</option>');
	                $.each(data, function(index, item) {
	                    rejectCodeSelect.append($('<option>', { value: item.code, text: item.name }));
	                });
	                rejectModal.css('display', 'flex');
	                setTimeout(() => rejectModal.addClass('visible'), 10);
	            },
	            error: function() {
	                alert('거절 사유 목록을 불러오는 데 실패했습니다.');
	            }
	        });
	    });
	    ...
	    $('#confirm-reject-btn').on('click', function() {
	        ...
	        $.ajax({
	            ...
	            error: function(xhr) {
	                ...
	                console.error("부지급 처리 AJAX Error: ", xhr.responseText);
	                alert(errorMsg);
	            }
	        });
	    });
	    ...
	    function closeModal() {
	        ...
	        setTimeout(() => { rejectModal.css('display', 'none'); }, 200);
	    }

	    $('#cancel-reject-btn').on('click', closeModal);
	    rejectModal.on('click', function(event) { if (event.target === this) closeModal(); });
	});
</script>
